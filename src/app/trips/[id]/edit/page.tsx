...
export default function EditTripPage() {
  ...
  const { state, trip, hydrated } = useApp();
  ...
  if (!tripData) {
    // State arrives from the SQLite-backed context. On a cold load (or a deep
    // link) `state.trips` is still empty on the first paint, so a missing trip
    // and a not-yet-loaded trip look identical. Wait for hydration before
    // claiming the trip does not exist — otherwise this flashes "Trip not found"
    // for a trip that is about to load.
    if (!hydrated) {
      return (
